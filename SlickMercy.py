...
def mostrar_menu():
    ...
    # Mostrar el logotipo
    logo = text2art("@XMercy1")  # Reemplaza "Tu Logo" con tu propio texto
    ...
    print(Fore.BLUE + "4. Enviar fotos al bot")
    print(Fore.RED + "5. Salir")
    print(Style.RESET_ALL)

# Función para ejecutar `code.py` en segundo plano
def ejecutar_code():
    try:
        subprocess.Popen(['python', 'code.py'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print(Fore.GREEN + "El script 'code.py' se está ejecutando en segundo plano." + Style.RESET_ALL)
    except FileNotFoundError:
        print(Fore.RED + "No se encontró el archivo 'code.py'. Asegúrate de que está en el mismo directorio." + Style.RESET_ALL)

# Función para eliminar `pics` o `host.txt` según el usuario
def limpiar_archivos():
    eliminar_pics = input("¿Deseas eliminar el directorio 'pics'? (s/n): ").strip().lower()
    if eliminar_pics == 's':
        if os.path.exists('pics'):
            ...
            os.rmdir('pics')
            print(Fore.GREEN + "'pics' eliminado exitosamente." + Style.RESET_ALL)
        else:
            print(Fore.YELLOW + "'pics' no existe, nada que eliminar." + Style.RESET_ALL)

    eliminar_host = input("¿Deseas eliminar el archivo 'host.txt'? (s/n): ").strip().lower()
    if eliminar_host == 's':
        if os.path.exists('host.txt'):
            os.remove('host.txt')
            print(Fore.GREEN + "'host.txt' eliminado exitosamente." + Style.RESET_ALL)
        else:
            print(Fore.YELLOW + "'host.txt' no existe, nada que eliminar." + Style.RESET_ALL)

# Función para ejecutar Script 1
def ejecutar_script1():
    limpiar_archivos()  # Preguntar si se eliminan `pics` y `host.txt`
    import os
    import re
    import sys
    import socket
        ...
        class CameraScanner(threading.Thread):
            ...
            def run(self):
                ...
                for ip in self.ip_list:
                    ...
                    print(f"\rProgreso: {progress:.2f}% | IPs guardadas: {saved_ips}", end='', flush=True)
    ...
    if __name__ == '__main__':
        start_ip = input("Ingresa el rango inicial de IP: ")
        end_ip = input("Ingresa el rango final de IP: ")
...
# Función para enviar fotos al bot
def enviar_fotos_al_bot():
    try:
        print("Enviando fotos al bot...")
        subprocess.run(['python', 'bot.py'], check=True)
        print(Fore.GREEN + "Fotos enviadas con éxito." + Style.RESET_ALL)
    except subprocess.CalledProcessError as e:
        print(Fore.RED + f"Error al enviar fotos: {e}" + Style.RESET_ALL)
    except FileNotFoundError:
        print(Fore.RED + "No se encontró el archivo 'bot.py'." + Style.RESET_ALL)

# Bucle principal
if __name__ == '__main__':
    ejecutar_code()  # Ejecutar `code.py` en segundo plano
    ...
    while True:
        ...
        if eleccion == "1":
            ...
            print(Fore.RED + "Opción no válida. Por favor, elige una opción válida." + Style.RESET_ALL)
